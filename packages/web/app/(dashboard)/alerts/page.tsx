...
export default function AlertsPage() {
  ...
  const { data: alerts, isLoading } = useQuery({
    ...
    queryFn: () =>
      alertApi.list({ status: statusFilter || undefined, severity: severityFilter || undefined }),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Alert Queue"
        title="Active alert operations"
        description="Triage, acknowledge, and silence incidents as new signals arrive from Prometheus and the API."
      >
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
        <SummaryStat
          ...
          tone={(alertStats?.firing || 0) > 0 ? 'danger' : 'default'}
        />
        <SummaryStat label="Critical" value={alertStats?.critical || 0} tone="danger" />
        <SummaryStat label="Warning" value={alertStats?.warning || 0} tone="warning" />
        <SummaryStat label="Acknowledged" value={alertStats?.acknowledged || 0} tone="primary" />
        <SummaryStat label="Silenced" value={alertStats?.silenced || 0} tone="primary" />
        <SummaryStat label="Resolved" value={alertStats?.resolved || 0} tone="success" />
      </div>
        ...
        <div className="flex items-center gap-4 p-3 bg-blue-500/10 dark:bg-blue-500/20 border border-blue-500/20 rounded-lg">
          ...
          {selectedFiringIds.length > 0 && (
            <>
              ...
              >
                {bulkSilenceMutation.isPending
                  ? 'Silencing...'
                  : `Silence Selected (${silenceLabel})`}
              </Button>
            </>
          )}
        ...
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              ...
              title="No alerts"
              description="No active alert signals are firing. System health is currently stable across monitored targets."
              icon={
                <svg
                        ...
                        (alert.status === 'ACKNOWLEDGED' || alert.status === 'SILENCED') && (
                          <p className="text-xs text-muted-foreground mt-1">
                            ...
                            {alert.acknowledgedAt && (
                              <> at {formatTimeOnly(alert.acknowledgedAt)}</>
                            )}
                          </p>
                        )}
                    </TableCell>
